...
const attendee = new Datastore({
  filename: dbPath + "/attendee.db",
  autoload: true,
});
const session = new Datastore({
  filename: dbPath + "/session.db",
  autoload: true,
});
...
let thismonth = (parseInt(currentDate.getMonth(), 10) + 1).toString();
...
function createWindow() {
  ...
  sessionWindow.loadURL(
    isDev
      ? "http://localhost:3000/#/sessioninfo"
      : `file://${path.join(__dirname, "index.html#/sessioninfo")}`
  );
  editWindow.loadURL(
    isDev
      ? "http://localhost:3000/#/editpage"
      : `file://${path.join(__dirname, "index.html#/editpage")}`
  );
  analyzerWindow.loadURL(
    isDev
      ? "http://localhost:3000/#/analyzer"
      : `file://${path.join(__dirname, "index.html#/analyzer")}`
  );
  tabsWindow.loadURL(
    isDev
      ? "http://localhost:3000/#/tabspage"
      : `file://${path.join(__dirname, "index.html#/tabspage")}`
  );
  ...
  /**
   * Set main window to null when closed.
   * Meaning the whole app will stop running
   */
  ...
  /**
   * Hide the view session window when closed.
   * This will help the window correctly open again without errors
   *  when opened again before closing the main window.
   */
  ...
  /**
   * Hide the tabs window when closed.
   * This will help the window correctly open again without errors
   * when opened again before closing the main window.
   */
  ...
  /**
   * Hide the edit window when closed.
   * This will help the window correctly open again without errors
   * when opened again before closing the main window.
   */
  ...
  /**
   * Hide the analyzer window when closed.
   * This will help the window correctly open again without errors
   * when opened again before closing the main window.
   */
  ...
  /**
   * Hide the settings window when closed.
   * This will help the window correctly open again without errors
   * when opened again before closing the main window.
   */
  ...
  /** Hide the about window when closed.
   * This will help the window correctly open again without errors
   * when opened again before closing the main window.
   */
...
/**
 * If the app opens again after closing the main window,
 * call the createWindow function again.
...
/**
 * event emitter to check if the application is unlocked
...
ipcMain.on("open-tabs", async (event, arg) => {
  ...
  await session.findOne({ _id: currDate }, async (err, doc) => {
     ...
     * if started, then return service already started as true to frontend.
    ...
    if (doc) {
      ...
      await putNewDate(startingTime, endingTime).catch((err) => {
        console.log(err);
      });
...
ipcMain.on("store-settings", async (event, arg) => {
  /**
   * insert or update the settings data according to the data
   * coming from frontend
   */
  const currentDate = arg.currentSession;
    ...
    if (doc) {
      ...
      await temp.find({ _id: 3 }, async (err, doc) => {
        if (err) return console.log(err);
          ...
          return await temp.update(
            { _id: 3 },
            { $set: { tempChurchName: arg.churchName } }
          );
        }
      });
...
ipcMain.on("new-attendee", async (event, arg) => {
  ...
  let firstName = arg.firstName.split(" ").join("");
  let lastName = arg.lastName.split(" ").join("");
  let gender = arg.gender.split(" ").join("");
  let location = arg.location;
  let contactNumber = arg.contactNumber.split(" ").join("");
  let emailAddress = arg.emailAddress.split(" ").join("");
  let temperature = arg.temperature.split(" ").join("");
  ...
  await attendee.find({}, async (err, doc) => {
    ...
    if (selectedAttendee.length === 0) {
      ...
      await newAttendee(generatedDetails).catch(function (err) {
        console.log(err);
      });
      ...
      let contactParam;
      let emailParam;
      for (let i = 0; i < selectedAttendee.length; i++) {
        firstParam = selectedAttendee[i].firstName
          .split(" ")
          .join("")
          .toLowerCase();
        secondParam = selectedAttendee[i].lastName
          .split(" ")
          .join("")
          .toLowerCase();
        genderParam = selectedAttendee[i].gender
          .split(" ")
          .join("")
          .toLowerCase();
        locationParam = selectedAttendee[i].location
          .split(" ")
          .join("")
          .toLowerCase();
        contactParam = selectedAttendee[i].contactNumber
          .split(" ")
          .join("")
          .toLowerCase();
        emailParam = selectedAttendee[i].emailAddress
          .split(" ")
          .join("")
          .toLowerCase();
        if (
          firstParam === firstName.toLowerCase() &&
          secondParam === lastName.toLowerCase() &&
          genderParam === gender.toLowerCase() &&
          locationParam === location.toLowerCase() &&
          contactParam === contactNumber.toLowerCase() &&
          emailParam === emailAddress.toLowerCase()
        ) {
 ...
 * Done by checking if the current date exists.
 * If it exists, there will be a current session started
 */
    ...
    if (err) return console.log(err);
      ...
      await session.findOne({ _id: theTempDate }, async (err, doc) => {
        if (err) return console.log(err);
          ...
          return event.reply(
            "verify-session-availability-reply",
            sessionAvailable
          );
        } else {
...
/**
 * event emitter used to search for all attendees
 *  who have been registered in the database
 * also used to send a the results of the search
 * in the form a reply to the frontend
 */
...
/** event emitter used to search for all attendees
 * who have been registered in the current session
 * also used to send a the results of the search
 *  in the form a reply to the frontend
 */
ipcMain.on("old-attendee-current-session-all", async (event, arg) => {
  await temp.findOne({ _id: 2 }, async (_err, sesObj) => {
    if (sesObj) {
      ...
      await session.findOne({ _id: tempDateValue }, async (err, doc) => {
        if (err) return console.log(err);
          ...
          await event.reply("sessionTime-reply", {
            start: doc.session.start,
            end: doc.session.end,
          });
...
ipcMain.on("view-old-attendee-current-session-all", async (event, arg) => {
  await temp.findOne({ _id: 1 }, async (_err, sesObj) => {
    if (sesObj) {
      ...
      await session.findOne({ _id: tempDateValue }, async (err, doc) => {
        if (err) return console.log(err);
          ...
          const gottenAttendees = doc.session.attendee;
          await event.reply("view-sessionTime-reply", {
            start: doc.session.start,
            end: doc.session.end,
          });
          return await event.reply(
            "view-old-attendee-current-session-all-reply",
            gottenAttendees
          );
        }
      });
...
/**
 * event emitter used to search for attendee(s)
 * whose name(s) correspond(s) to the search parameter
 * also used to send the results of the search
 * in the form of a reply to the frontend
 */
ipcMain.on("old-attendee-name", async (event, arg) => {
  const searchName = arg.toLowerCase();
  /**
   * search for attendee with the search parameter
   * in either their first name or last name
   */
  await attendee.find({}, async (err, nameObj) => {
    if (err) return console.log(err);
      ...
      for (let j = 0; j < gottenSessions.length; j++) {
        if (
          gottenSessions[j].firstName.toLowerCase().indexOf(searchName) >= 0 ||
          gottenSessions[j].lastName.toLowerCase().indexOf(searchName) >= 0
        ) {
    ...
    if (sesObj) {
      ...
      await session.findOne({ _id: tempDateValue }, async (err, doc) => {
        if (err) return console.log(err);
          ...
          const currentArray = [];
          const gottenAttendees = doc.session.attendee;
          for (let j = 0; j < gottenAttendees.length; j++) {
            if (
              gottenAttendees[j].firstName.toLowerCase().indexOf(searchName) >=
                0 ||
              gottenAttendees[j].lastName.toLowerCase().indexOf(searchName) >= 0
            ) {
...
ipcMain.on("verify-account-key", async (event, arg) => {
  ...
  if (
    verificationKey ===
    "mini->coTraCkdev&Prod.BycTeCh202160422724181510mm!&b@bLn"
  ) {
    temp.findOne({ _id: 4 }, async (err, doc) => {
      if (err) return console.log(err);
      else if (doc) {
...
/**
 * record an old attendee's temperature
 * when this event emitter is on or activated
 */
  ...
  return await temp.findOne({ _id: 2 }, async (err, doc) => {
    if (err) return console.log(err);
      ...
      return await session.findOne({ _id: theTempDate }, async (err, doc) => {
        if (err) return console.log(err);
        /**
         * check if the attendee to record his/her temperature
         * is not already stored in the current session.
         * call the recordAttendeeTemperature function
         * and send a "false" reply to frontend
         * if the attendee does not exist already in the current session
         */
        const theAttendeesArray = doc.session;
        if (theAttendeesArray.attendee.length === 0) {
          ...
          for (let j = 0; j < theAttendeesArray.attendee.length; j++) {
            if (
              theAttendeesArray.attendee[j].firstName === firstName &&
              theAttendeesArray.attendee[j].lastName === lastName &&
              theAttendeesArray.attendee[j].gender === gender &&
              theAttendeesArray.attendee[j].location === location &&
              theAttendeesArray.attendee[j].contactNumber === contactNumber &&
              theAttendeesArray.attendee[j].emailAddress === emailAddress
            ) {
 ...
 * Data like number of attendee for that date,
 * number of males and number of females go with it
 */
ipcMain.on("search-tempDate", async (event, arg) => {
  await temp.findOne({ _id: 1 }, async (err, doc) => {
    if (err) return console.log(err);
      ...
      await session.findOne({ _id: tempViewDate }, async (_err, doc) => {
        if (doc) {
          /**
           * loop through the attendee array to
           * check the number of males and females
           */
          let maleNum = 0;
          let femaleNum = 0;
          const tempHolder = doc.session.attendee;
          for (let j = 0; j < tempHolder.length; j++) {
            if (tempHolder[j].gender === "Male") maleNum += 1;
            else femaleNum += 1;
          }

          const viewSessionDetails = {
            theTempDate: tempViewDate,
            theTotal: tempHolder.length,
            theMales: maleNum,
            theFemales: femaleNum,
          };
...
ipcMain.on("analysis-data", async (event, arg) => {
  await session.find({}, async (_err, doc) => {
    if (doc) {
      const tempDoc = [];
      let tempHolder;
      for (let k = 0; k < doc.length; k++) {
        /**
         * push each object of the analysis data
         * into the array "tempDoc" using the format below
         */
        await tempDoc.push({
          _id: doc[k]._id,
          maleNum: 0,
          femaleNum: 0,
          start: doc[k].session.start,
          end: doc[k].session.end,
          attendeeeNum: doc[k].session.attendee.length,
        });
      }
      /**
       * loop through the data of all sessions
       * to check the number of males and females
       */
...
/**
 * event emitter used to search for the service information
 * that correspond(s) to the search parameter
 * also used to send the results of the search
 * in the form of a reply to the frontend
 */
ipcMain.on("analysis-data-search", async (event, arg) => {
  ...
  await session.find({}, async (err, doc) => {
    if (err) return console.log(err);
      ...
      const currentArray = [];
      for (let l = 0; l < tempDoc.length; l++) {
...
ipcMain.on("add-attendee-details", async (event, arg) => {
  await attendee.insert({
    firstName: arg.editFirstName,
    lastName: arg.editLastName,
    gender: arg.editGender,
    location: arg.editLocation,
    contactNumber: arg.editContactNumber,
    emailAddress: arg.editEmailAddress,
  });
...
ipcMain.on("open-updates", async (event, arg) => {
  await shell.openExternal(
    "https://clemotecghana.000webhostapp.com#clemotec-apps"
  );
});
...
async function newAttendee(data) {
  ...
  return await temp.findOne({ _id: 2 }, async (err, doc) => {
    if (err) return console.log(err);
      ...
      const theTempDate = doc.tempDate;
      /**
       * Add the new attendee's information
       * (temperature, first name, last name etc..) to the
       * current church service or to the service set in settings
       */
      await session.findOne({ _id: theTempDate }, async (err, doc) => {
        if (err) return console.log(err);
...
async function tempDate(viewSessionDate) {
  ...
  await temp.find({ _id: 1 }, async (err, doc) => {
    if (err) return console.log(err);
      ...
      return await temp.update(
        { _id: 1 },
        { $set: { tempViewDate: viewSessionDate } }
      );
    }
  });
}
      ...
      if (fs.existsSync(downloadsPath + "/Cotrack-mini")) {
        ...
        await fs.mkdir(downloadsPath + "/Cotrack-mini", async (err) => {
          if (err) return console.log(err);
            ...
            const pdfPath =
              downloadsPath + "/Cotrack-mini/" + tempDate + ".pdf";
            // options for the pdf to be printed
            const options = {
              marginsType: 0,
              printBackground: false,
            ...
            await sessionWindow.webContents
              .printToPDF(options)
              .then((pdfFile) => {
                fs.writeFile(pdfPath, pdfFile, (err) => {
                  if (err) return console.log(err);
...
const generalTemplate = [
  ...
  {
    label: "Help",
    submenu: [{ label: "About", click: () => showAboutPage() }],
  },
];
...
const printTemplate = [
  ...
  },
  {
    ...
    submenu: [
      {
        label: "as PDF",
        /* icon: 'src/Assets/images/printer1.png', */ click: () =>
          takeCareOfPDFPrinting(),
      },
