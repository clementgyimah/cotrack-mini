...
export default function SettingsPage() {
  ...
  /**
   * expression that checks the validity of the updated settings
   * data before prompting backend to save the data
   */
  const onSubmit = () => {
    if (
      churchName.length === 0 ||
      churchName.length > 50 ||
      currentSession.length === 0 ||
      parseInt(currentSession, 10) <= 0
    ) {
    ...
    <div className="settings-container">
      ...
      <div className="settings-content">
        ...
        <div className="settings-content-row-div">
          <label htmlFor="church-name-label" className="settings-details-text">
            Organization Name:{" "}
          </label>
          <input
            type="text"
            id="church-name-label"
            className="details-input-text"
            placeholder="Enter Church Name"
            value={churchName}
            onChange={churchNameFunc}
          />
        </div>
        <div className="settings-content-row-div">
          <label
            htmlFor="current-session-label"
            className="settings-details-text"
          >
            Current Session:{" "}
          </label>
          <input
            type="date"
            id="current-session-label"
            className="details-input-text"
            placeholder="2020-09-15"
            value={currentSession}
            onChange={currentSessionFunc}
          />
        </div>
        ...
        {invalidInput ? (
          ...
          <div>
            ...
            <div className="settings-error-hint">
              <b>Hint: </b>
            </div>
            <div className="settings-error-hint">
              1. Make sure there is or was attendance recorded on:{" "}
